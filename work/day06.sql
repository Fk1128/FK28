...
--CASE와 WHEN사이에 비교하고자 하는 컬럼을넣고
-- WHEN과 THEN사이에 비교하고자 하는 값을 넣어서 비교하는 방법

-- EMP테이블에서 SAL이 3천이상이면 HIGH 천 이상이면 MID 둘다 아니면 LOW를 
--사원명 (ENAME) 급여 SAL 등급 GRADE순으로 조회하세ㅛ
...
-- STADIUM테이블에서 SEAT_COUNT(좌석수)가 0이상30000 이하면 'S'
...
-- 이외에는 'H' 선수이름 ,몸무게(단위추가),크기순으로 조회 

SELECT PLAYER_NAME AS 선수이름 ,WEIGHT||'KG' AS 몸무게,
...
AGE NUMBER(3) := 30 ;
BEGIN DBMS_OUTPUT.PUT_LINE('이름 :' ||NAME||CHR(10) || '나이:' || AGE);
END;
...
BEGIN
		FOR I IN 1..4 LOOP
			IF MOD(I,2)=0 THEN
				DBMS_OUTPUT.PUT_LINE(I||'는짝수!');
			ELSE
				DBMS_OUTPUT.PUT_LINE(I||'는홀수!');
			END IF ;
		END LOOP;		
END;
